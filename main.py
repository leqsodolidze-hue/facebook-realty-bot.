...
user_lang = {} # მომხმარებლის ენა აქ დავიმახსოვრებთ
            ...
            for event in entry.get('messaging', []):
                ...
                if 'message' in event and 'text' in event['message']:
                    ...
                    if text in ["start", "გამარჯობა", "gamarjoba", "hello", "zdravstvuyte", "привет"]:
                        ...
                        msg = "მენიუსთვის დაწერეთ START" if lang=="ka" else "Type START for menu" if lang=="en" else "Напишите START для меню"
...
def handle_postback(sender_id, payload):
    ...
    if payload.startswith("LANG_"):
        ...
        send_language_buttons(sender_id) # START-ზე პირდაპირ ენები
    elif payload == "BUY":
        lang = user_lang.get(sender_id, "ka")
        t = TEXTS[lang]
        ...
        lang = user_lang.get(sender_id, "ka")
        t = TEXTS[lang]
        send_text(sender_id, t["warning"] + "\n" + ("ქირის განყოფილება მალე დაემატება" if lang=="ka" else "Rent section coming soon" if lang=="en" else "Раздел аренды скоро"))
    elif payload == "SELL":
        lang = user_lang.get(sender_id, "ka")
        t = TEXTS[lang]
        send_text(sender_id, t["warning"])
...
def show_listings(sender_id, city):
    ...
    # ვაჩვენებთ მხოლოდ პირველ 3-ს
    for item in listings[:3]:
        send_text(sender_id, f"🏠 {item['title']}\n💰 ფასი: {item['price']}\n🔗 {item['link']}")

    # თუ მეტია, ვაჩვენებთ ღილაკს
    if len(listings) > 3:
